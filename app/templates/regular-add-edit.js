...
import { Checkbox, TimePicker, Select, Cascader,message   } from 'antd';
...
const { Option } = Select;
class RegularCourseAddEditTemplate extends CommonPage {
    constructor(props){
        super(props);
        this.state = {
            activeKey: 0,
    ...
    render(){
        ...
        return (
            <CommonPageComponent table={false} className="q-sub-page" title={pageConfig.title} pageFooter={footerConfig}>
                                                            ...
                                                            <div className="q-timepicker-group" key={i}>
                                                                <Checkbox value={'' + i} key={i}>
                                                                    <TimePicker
                                                                        ...
                                                                        value={moment(detail.course_begin, 'HH:mm')}
                                ...
                                <section className="q-sub-section">
                                    ...
                                    <div className="q-sub-section-main">
                                        <FormComponent direction="vertical" labelWidth={this.labelWidth}>
                                            ...
                                            <FormComponent.Group label="课程节数">
                                                <InputComponent
                                                    placeholder="请输入课程节数"
                                                    help="输入整数，如：20"
                                                    error={errors.count}
                                                    value={params.count}
                                                    onChange={(e) => this.controlCount(e.target.value)}
                                                    onBlur={(e) => this.settleCountError(e.target.value)}
                                                />
                                            </FormComponent.Group>
                ...
                {goBack ? <Redirect to='/course/regular/list' push={true}/> : null}
            </CommonPageComponent>
        )
    }
    componentDidMount(){
    ...
    controlTimeList(index, type, time){
        this.setState(prev => {
            ...
            details[index][type] = time.format('HH:mm');
            return {
                params: {
                    ...prev.params,
    ...
    settleCountError(value){
        if(!value){
            this.settleErrors('count', ERROR.courseCount.required);
        }else if(!numberValidator.int(value)){
            this.settleErrors('count', ERROR.courseCount.int);
        }else{
            this.settleErrors('count', null);
        }
    }
    controlCount(value){
        this.settleParams('count', value);
        this.settleCountError(value);
    }
    controlPhone(value){
    ...
    disabledBtn(){
        const {name, course_detail, address,cost,count, tel, trainer_id} = this.state.params;
        let bool = false;
        ...
        return !(!!name && !!address && !!cost && numberValidator.int(count) && phoneValidator.cell(tel) && !!trainer_id);
    }
}
export default RegularCourseAddEditTemplate;